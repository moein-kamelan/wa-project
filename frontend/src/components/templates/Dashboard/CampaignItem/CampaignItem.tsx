...
function CampaignItem({
  ...
  const { mutate: changeCampaignTitle } = usePutCampaignTitle(page);
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        <button
          className={`shrink-0 text-primary hover:rounded-full hover:bg-secondary p-1  hover:shadow-[2px_2px_4px_0px_rgba(0,0,0,0.25)] hover:text-white ${
            editable
              ? "bg-secondary rounded-full shadow-[2px_2px_4px_0px_rgba(0,0,0,0.25)] text-white"
              : ""
          }`}
        >
          <svg
            onClick={() => {
              ...
              if (title !== campaign.title) {
                changeCampaignTitle({ campaignId: campaign.id, title });
              }
            }}
